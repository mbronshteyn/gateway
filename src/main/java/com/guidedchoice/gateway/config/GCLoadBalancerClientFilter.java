...
public class GCLoadBalancerClientFilter extends LoadBalancerClientFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            if (urlPort > 0) {
                // rebuild url using port passed
                String newUrl = String.format("%s://%s:%s%s", scheme, host, urlPort, "/actuator/health");
                requestUrl = new URI(newUrl);
            } else if (url.getPath().contains("health")) {
                String newUrl = String.format("%s://%s:%s%s", scheme, host, "8081", "/actuator/health");
                requestUrl = new URI(newUrl);
            }
